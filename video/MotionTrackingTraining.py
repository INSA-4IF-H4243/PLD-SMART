...
video_title = "67"
videoPath = '../dataset/' + video_title + '.mp4'  # chemin de la video
outPutPathJHaut = '/' + video_title  # chemin d'enregistrement de la silouhette du Joueur 1
outPutPathJBas = '/' + video_title  # chemin d'enregistrement de la silouhette du Joueur 2
outPutPath = "../img/"  # ex : avec les 3 outputs paths cela donnera : img/JHaut/nom_coup/outPutPathJHaut/liste des
                     # images du coup
...
# taille de lentree du machine learning pour une seconde= fpsOutput * [PixelSizeOutput * PixelSizeOutput] (
# 20*20*20=8000 pixels noir ou blanc)
tableauSortieJHaut = []
tableauSortieJBas = []
...
while cap.isOpened() and ret3:  # attention video qui s'arete au premier probleme dans la lecture a cause du resize
    ...
    if (nbFrame % rapportFps < 1):
        ...
        cv2.rectangle(frame1, (affichageJHaut[0], affichageJHaut[1]),
                      (affichageJHaut[0] + affichageJHaut[2], affichageJHaut[1] + affichageJHaut[3]), (0, 200, 0), 2)
        (x1, y1, w1, h1) = joueurs[1]  # Joueur 1 du bas
        affichageJBas = (x1 - decalageX, y1 - decalageY, w1 + 2 * decalageX, h1 + 2 * decalageY)
        cv2.rectangle(frame1, (affichageJBas[0], affichageJBas[1]),
                      (affichageJBas[0] + affichageJBas[2], affichageJBas[1] + affichageJBas[3]), (0, 255, 0), 2)
