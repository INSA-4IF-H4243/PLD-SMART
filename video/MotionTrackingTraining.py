...
videoName = '21'
videoPath = '../dataset/' + videoName + '.mp4'  # chemin de la video
outPutPathJHaut = videoName + "_"  # chemin d'enregistrement de la silouhette du Joueur 1
outPutPathJBas = videoName + "_"  # chemin d'enregistrement de la silouhette du Joueur 2
...
def similarite(rec1, rec2):
    distance = (centre(rec1)[0] - centre(rec2)[0]) * (centre(rec1)[0] - centre(rec2)[0]) + (
                centre(rec1)[1] - centre(rec2)[1]) * (centre(rec1)[1] - centre(rec2)[1])
...
while cap.isOpened() and ret3:  # attention video qui s'arete au premier probleme dans la lecture a cause du resize
    ...
    cv2.line(transformations[0], (xmax - 150, int(a_left * (xmax - 150) + b_left)),
             (xmax, int(a_left * (xmax) + b_left)), (255, 137, 0), 2)
    cv2.line(transformations[0], (xmin, int(a_right * (xmin) + b_right)),
             (xmin + 150, int(a_right * (xmin + 150) + b_right)), (137, 255, 0), 2)
    ...
    for contour in contours:
        ...
        if area > 1:
            ...
            if area > 300 and (not (h < 20 or w < 20)) and (y_c + ymin) > a_left * (x_c + xmin) + b_left and (
                    y_c + ymin) > a_right * (x_c + xmin) + b_right:
                cv2.rectangle(transformations[0], (x_c + xmin, y_c + ymin), (x_c + xmin + 5, y_c + ymin + 5),
                              (0, 0, 255), 2)
    ...
    if balle_detecte:
        for rec in ball_rec:
            if distance2(balle, rec) < distance2(balle, minBalle) and distance2(balle, rec) < 50 and distance2(balle,
                                                                                                               rec) > 1:
                minBalle = rec
                bBalle = 1
        if bBalle:
    ...
    if not balle_detecte:
        if compteur_non_detection < limite:
            for rec in ball_rec:
                if (distance2(balle, rec) < distance2(balle, minBalle) and distance2(balle,
                                                                                     rec) < 50 + rayon_detection * (
            ...
            for rec in ball_rec:
                if not b and ((distance2(joueurs[0], rec) < distance2(joueurs[0], minBalle) and distance2(joueurs[0],
                                                                                                          rec) < 100 and
                               joueurs[0][1] - 20 < rec[1]) or (
                                      distance2(joueurs[1], rec) < distance2(joueurs[1], minBalle) and distance2(
                                      joueurs[1], rec) < 100 and joueurs[1][1] + joueurs[1][3] + 20 > rec[1])):
                    minBalle = rec
                    bBalle = 1
            ...
            if tableau_trajectoire_balle[i] == (-1, -1):
                ...
                if no_pos > 0:
                    if no_debut:
                        ...
                        for j in range(1, no_pos + 1):
                            x = av_derniere_pos_balle[0] + int(
                                ((derniere_pos_balle[0] - av_derniere_pos_balle[0]) / no_pos) * j)
                            y = av_derniere_pos_balle[1] + int(
                                ((derniere_pos_balle[1] - av_derniere_pos_balle[1]) / no_pos) * j)
    ...
    for joueur in joueurs:
        cv2.rectangle(
            transformations[0], (joueur[0] + xmin - 10, joueur[1] + ymin - 10),
            (joueur[0] + xmin + joueur[2], joueur[1] + ymin + joueur[3]), (0, 0, 255), 2
        )  # players (x,y)(x1,y1) -> (x,y,w,h)
    if balle_detecte:
        cv2.rectangle(
            transformations[0], (balle[0] + xmin - 10, balle[1] + ymin - 10),
            (balle[0] + xmin + balle[2], balle[1] + ymin + balle[3]), (0, 255, 0), 2
        )  # ball
    ...
    if (nbFrame % rapportFps < 1):
        ...
        try:
            ...
            silouhetteBas = transfomations_joueurs[3][max(0, joueurs[1][1] - 15):joueurs[1][1] + joueurs[1][3] + 30,
                            max(0, joueurs[1][0] - 15):joueurs[1][0] + joueurs[1][2] + 30]
        ...
        try:
            silouhetteHaut = transfomations_joueurs[3][max(joueurs[0][1] - 15, 0):joueurs[0][1] + joueurs[0][3] + 30,
                             max(0, joueurs[0][0] - 15):joueurs[0][0] + joueurs[0][2] + 30]
        ...
        if (k_pressed == True):
            ...
            if (coupJHaut): imageProcessor.save_ImageList(
                tableauSortieJHaut[len(tableauSortieJHaut) - cutFrameNB:len(tableauSortieJHaut)],
                outPutPath + "JHaut/" + tabCoups[coupJHaut] + outPutPathJHaut + str(nbFrame), enregistrementImage)
            if (coupJBas): imageProcessor.save_ImageList(
                tableauSortieJBas[len(tableauSortieJHaut) - cutFrameNB:len(tableauSortieJHaut)],
                outPutPath + "JBas/" + tabCoups[coupJBas] + outPutPathJHaut + str(nbFrame), enregistrementImage)
