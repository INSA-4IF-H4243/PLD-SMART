...
class ModelJoueurClassique:
    """
    Classic neural network model for the player
    """
    ...
    @classmethod
    def load_model_from_path(cls, model_path: str) -> 'ModelJoueurClassique':
        """
    ...
    @classmethod
    def load_model_from_layers(cls, layers: list) -> 'ModelJoueurClassique':
        """
                ...
                for file in os.listdir(path_fol):
                    ...
                    if os.path.isdir(path_fol_img) and file == 'images':
                        ...
                        for file_img in os.listdir(path_fol_img):
                            ...
                            img_obj = Image.load_image(
                                cv2.IMREAD_GRAYSCALE, path_img)
                            img = img_obj.img
                            frames.append(img)
    ...
    def split_train_test(self, list_videos, y, random_state: int = 42, test_size: float = 0.2,
        ...
        X_train = np.zeros((train_size, shape_frame[0]*shape_frame[1]*nb_frame), dtype=int)
        for i, vid in enumerate(vids_train):
            seq_img = vid.frames.flatten()
            X_train[i] = seq_img

        X_test = np.zeros((test_size, shape_frame[0]*shape_frame[1]*nb_frame), dtype=int)
    ...
    def train(self, X_train, y_train, random_state: int = 1234, learning_rate: float = 0.001,
        ...
        self.model.compile(
            ...
            optimizer=tf.keras.optimizers.RMSprop(learning_rate=learning_rate),  # 'Adam',
            metrics=['accuracy']
        )
