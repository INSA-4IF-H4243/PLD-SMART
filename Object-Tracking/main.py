...
while exist:
    ...
    contours, hierarchy = cv2.findContours(transformations[-1], cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    for contour in contours:
        area = cv2.contourArea(contour)
        if area > 1:
            x, y, w, h = cv2.boundingRect(contour)
            if area > 20:
                cv2.rectangle(transformations[0], (x - 10, y - 10), (x + w, y + h), (0, 0, 255), 2) # players
            else:
                cv2.rectangle(transformations[0], (x - 10, y - 10), (x + w, y + h), (0, 255, 0), 2) # ball
